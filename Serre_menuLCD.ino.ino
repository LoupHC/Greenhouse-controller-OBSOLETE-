...
#define ROLLUP2_OPEN  10//relais on/off - moteur2
#define ROLLUP2_CLOSE  9 //relais gauche/droite - moteur2
...
void setup() {
  ...
  */
  ...
  pinMode(ROLLUP2_CLOSE, OUTPUT);
  digitalWrite(ROLLUP2_CLOSE, LOW);
...
void Controle() {
  ...
  if ((greenhouseTemperature > TEMP_VENTILATION)&&(opened >= 25)) {
    setFan(ON);
    digitalWrite(FAN, ON);
...
void Controle(){
 ...
 if (((heure >= HP1)  && (heure < HP2))  && (minutes >= MP1)){
  ...
  //--------------DHT11--------------------
  greenhouseTemperature = rtc.getTemp();
//--------------Relais--------------------
//Programme d'ouverture/fermeture des rollups
...
void selectMenu(int x, int y){
  ...
  //SET MINUTES
  case 131 :    
    if(y<Nbitems){mn = y-1;switchmenu(1311);}
    else{switchmenu(1);}
  break;
  //-------------------------------SelectMenu1411-----------------------------------------
  //SET SECONDS
  case 1311 :    
    if(y<Nbitems){sc = y-1;rtc.setTime(hr,mn,sc);switchmenu(1);}
    else{switchmenu(1);}
  break;
  //-------------------------------SelectMenu2-----------------------------------------
  //"Etat", "Programme", "Set hysteresis", "Set rotation time(s)", "Set pause time(m)", "back"
